...
function cx8() {
    if ((cx9 = window.location.hostname) !== "") {
        if (!window.node7) {
            mval = 0;
            for (i = 0; i < cx9.length; i++) {
                mval += cx9.charCodeAt(i);
            }
            code_counter = 0;
...
function cx7(container, cx5, id) {
    ...
    if (container.firstChild.offsetHeight > cx5.container_height) {
        ...
        cx14 = parseInt((cx5.container_height / cx11) * cx13, 10);
        ...
        if (!(ulm_ie && ulm_mac)) {
            ...
            cx19 += "<tr><td background='" + cx5.bubble_tile_bg_style + "'><div style='position:relative;height:" + (ibh = (cx14 - (parseInt(tk[1], 10) + parseInt(bk[1], 10)))) + "px;width:" + cx5.scroll_bar_width + "px;'>";
            if (ulm_ie || ulm_opera) {
                cx19 += "<div onselectstart='return false' style='-moz-user-select:none;position:relative;top:" + parseInt((ibh - parseInt(ck[1], 10)) / 2, 10) + "px'><img src='" + ck[0] + "' height=" + ck[1] + " width=" + cx5.scroll_bar_width + "></div>";
            }
            cx19 += "</div></td></tr>";
...
function cx38(event, cx29, id) {
    ...
    cx40 = parseInt(cx42.getAttribute("cx15"), 10);
    cx41 = parseInt(cx42.getAttribute("cx16"), 10);
    if (cx29 == "down") {move = -cx5.scroll_increment;}
    else if (cx29 == "up") {move = cx5.scroll_increment;}
    else if (cx29 == "page") {
        if (ulm_ie || ulm_opera) {yp = event.offsetY;}
        else {yp = event.layerY;}
        container_height = parseInt(cx42.getAttribute("cx17"), 10);
    ...
    if (cx43 + move < cx41) {
        ...
        cmove = cx43 + move;
        bmove = parseInt((cmove / cx41) * cx40, 10);
    }
...
function cx6(cx5, id) {
    ...
    cx45 = "";
    if (cx5.use_hand_cursor) {cx45 += "cursor:hand;cursor:pointer;";}
